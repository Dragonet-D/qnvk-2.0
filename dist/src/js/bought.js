...
HasBought.prototype.init = function () {
  ...
  $.ajax({
    ...
    success: function success(data) {
      console.log(data);
      $('body').html('\n        <div id="tab">\n          <div class="tab tab_active">\u7EBF\u4E0A\u8BFE\u7A0B</div>\n          <div class="tab">\u6D3B\u52A8\u95E8\u7968</div>\n        </div>\n        <ul class="tab_content">\n        </ul>\n        <ul class="tab_content tickets" style="display: none">\n        </ul>\n      ');
      if (data.length > 0) {
        ...
        This.bought.append('<li class="bought_content">' + '<h3 class="title">' + '<span class="class_name">课程包</span>' + '<span class="status">已购买</span>' + '</h3>' + '<a class="class_list" href="javascript:">' + '<img src="' + picture + '" alt="">' + '<span class="class_info">' + '<span class="class_title">' + title + '</span>' + '<span class="class_user">' + name + '</span>' + '</span>' + '<span class="price">￥' + price + '</span>' + '</a>' + '</li>');
      } else {
        This.bought.html('<li class="bought_record">' + '<span>您还未购买任何课程</span>' + '</li>');
      }
    }
  });
};
