...
AdAndTabs.prototype.getAds = function (adlist) {
  for (var i = 0; i < adlist.length; i++) {
    ...
    this.adWrapper.append('<div class="swiper-slide">' + '<a href="' + adurl + '">' + '<img src="' + imgUrl + '" alt="">' + '</a>' + '<h2 class="swiper_title">' + adjieshao + '</h2>' + '</div>');
  }
};
// 默认推荐选项卡内容
AdAndTabs.prototype.defaultTagContent = function (list) {
  for (var i = 0; i < list.length; i++) {
    if (list[i].tag1 !== ' ') {
      ...
      var id = list[i].id;
      if (title) {
        this.tabwrapper.append('<div class="swiper-slide" data-id="' + id + '">' + '<span>' + title + '</span>' + '</div>');
      }
    }
  }
};
  ...
  _createClass(Guidance, [{
    ...
    value: function tabClick() {
      ...
      this.swiper_slide.on('click', function () {
        var swiper_text = $(this).children('span').text();
...
function tagContent(tabInfo) {
  ...
  for (var i = 0; i < tabInfo.length; i++) {
    ...
    if (categoryname === '心理健康课' || categoryname === '心理健康课') {
      homeadvance.append('<div id="audio_advance" class="audio_advance choose_craful">' + '<h2 class="audio_title"></h2>' + '<ul class="audio_content"></ul>' + '</div>');
      ...
      for (var k = 0; k < blockTitle.length; k++) {
        ...
        audiocontent.append('<li class="audio_item">' + '<a class="audio_wrapper history" data-id="' + id + '" href="' + hosturl + '/index.php/Index/livedeil_0/id/' + id + '">' + '<i class="icon">' + '<img src="' + picture + '" alt="">' + '</i>' + '<span class="audio_info">' + '<span class="audio_title">' + audiotitle + '</span>' + '<span class="audio_owner">' + name + '</span>' + '<span class="audio_hot">' + '<time>' + time + '</time>' + '</span>' + '</span>' + '</a>' + '</li>');
      }
    } else if (categoryname === '精品课程包') {
      console.log('精品课程包');
      homeadvance.append('<div class="class_bag choose_craful">\n            <h2>' + categoryname + '</h2>\n            <ul class="bag_content">    \n            </ul>\n          </div>');
      ...
      for (var j = 0; j < blockTitle.length; j++) {
        ...
        str += '<li class="single_video">' + '<a class="single_list_wrapper history" data-id="' + ids + '" href="' + hosturl + '/index.php/' + playurl + '/id/' + ids + '">' + '<section class="list_header">' + '<span class="list_header_info">' + '<img class="class_type" src="' + logourl + '" alt="">' + '<p class="is_free">免费</p>' + '</span>' + '<img src="' + pictures + '" alt="">' + '<span class="title">' + '<title class="video_introduce">' + audiotitles + '</title>' + '</span>' + '</section>' + '<p class="video_owner">' + names + '</p>' + '</a>' + '</li>';
      }
      homeadvance.append('<div class="video_audio choose_craful">' + '<h2>' + categoryname + '</h2>' + '<ul class="list_content">' + str + '</ul>' + '</div>');
    }
  }
  setTimeout(function () {
    loading('none');
...
function liveInfo(liveinfo) {
  ...
  if (liveoff === 1) {
    ...
    classadvanve.html('<li class="live">' + '<a  class="history" data-id="' + id + '" href="' + hosturl + '/index.php/' + liveurl + '/id/' + id + '">' + '<span class="list_header">' + '<span class="list_header_info">' + '<img class="class_type" src="' + livetagsrc + '" alt="">' + '<p class="is_free">免费</p>' + '</span>' + '<img class="live_img" src="' + picture + '" alt="">' + '<span class="title all-cut-time">' + '</span>' + '</span>' + '<span class="live_introdus">' + '<span class="title">' + title + '</span>' + '<span class="live_user">' + spker + '</span>' + '<span id="living_mark" class="status">' + willlivetime + '</span>' + '</span>' + '</a>' + '</li>');
  }
...
function countDown(intDiff, living) {
  ...
  timer = setInterval(function () {
    ...
    if (intDiff > 0) {
      ...
      allCutTime.html('<span class="video_introduce">直播进行中...</span>');
    ...
    allCutTime.html('<span>倒计时：</span>' + '<span>' + day + '天' + hour + '时' + minute + '分' + second + '秒' + '</span>');
    intDiff--;
  }, 1000);
}
